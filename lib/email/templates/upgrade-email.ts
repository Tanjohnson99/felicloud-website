 ...
 * Sent when an existing account is upgraded to a new plan
 * NO PASSWORD included (user keeps existing password)
 */

import { renderBaseEmailLayout } from './base-layout';
import { getUpgradeEmailTranslations } from '../translations/upgrade-email';

interface UpgradeEmailData {
  username: string;
  displayName?: string;
  newQuota: string; // e.g., "1TB", "500GB"
  newPlan: string; // e.g., "1TB_Lifetime", "500GB_Monthly"
  locale?: string;
}
...
export function renderUpgradeEmail({
  ...
  newPlan,
  locale = 'en',
}: UpgradeEmailData): { subject: string; html: string } {
  const t = getUpgradeEmailTranslations(locale);
  ...
  const body = `
    ...
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin: 0 0 32px; background-color: #F3F4F6; border-radius: 8px; border: 1px solid #E5E7EB;">
      <tr>
        <td style="padding: 24px;">
          ...
          <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
            <tr>
              <td style="padding: 8px 0;">
                ...
                <span style="display: block; font-size: 24px; font-weight: 700; color: #2B7FFF; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
                  ${newQuota}
                </span>
              </td>
            </tr>
          ...
          <p style="margin: 16px 0 0; font-size: 14px; line-height: 20px; color: #059669; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
            ✅ ${t.passwordNote}
          </p>
        </td>
      </tr>
    ...
    <div style="margin: 0 0 32px; padding: 24px; background-color: #EFF6FF; border-radius: 8px; border-left: 4px solid #4A90E2;">
      ...
      <ol style="margin: 0; padding-left: 24px; color: #374151; font-size: 15px; line-height: 24px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
        ...
        <li style="margin-bottom: 0;">${t.step3}</li>
      </ol>
    </div>
